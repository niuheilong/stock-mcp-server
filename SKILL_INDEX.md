...
**⚠️ 重要提醒：**
当用户要求搜索信息时，**不要**先尝试 web_search 或 web_fetch，**直接先用 Chrome MCP**！
- web_search 经常因缺少 API key 失败
- web_fetch 无法处理动态网页
- Chrome MCP 零Token消耗，功能最全
...
【立即使用 Chrome MCP】
    ↓
browser(action="open", url="https://www.google.com/search?q=...")
    ↓
获取结果 → 返回给用户
```
...
### 需要记忆的关键点
1. **Chrome MCP 已配置** - 端口9222，随时可用
2. **零Token消耗** - 搜索抓取不花钱
3. **功能最全** - 比 web_search/web_fetch 都强
4. **优先使用** - 不要犹豫，直接用！
...
### ⚠️ 历史问题 (2026-03-18 22:00)
**事件**: 卡壳导致会话崩溃，丢失当天所有记忆
**原因**: 
1. 被动检查 - 只在显式调用时检查
2. 大记忆文件写入时卡住
3. 阈值过于宽松（Python对象>50万才报警）
4. 没有记忆文件备份机制
...
| 指标 | 旧阈值 | **新阈值** | 动作 | 说明 |
|-----|--------|-----------|------|------|
| **Python对象数** | 50,000 | **15,000** | 强制GC+清理 | 正常<8K，超过15K立即处理 |
| **会话对象增长** | 无 | **2,000** | 警告 | 单条消息增长过快 |
| **工作区大小** | 10MB | **8MB** | 清理缓存 | 当前2.8MB，提前预警 |
| **记忆文件大小** | 无 | **500KB** | 自动分片+备份 | 大文件是卡壳主因 |
| **浏览器页面** | 5个 | **3个** | 立即关闭 | 更激进 |
| **单消息处理** | 无 | **60秒** | 警告 | 防止死循环 |
| **会话运行时间** | 2小时 | **1小时** | 建议重启 | 防止上下文膨胀 |

### 核心改进 v2.0

#### 1. 增强健康检查 (`health_check_v2.py`)
```bash
# 完整检查并自动清理
python3 health_check_v2.py

# 快速检查（每次任务后调用）
python3 health_check_v2.py --quick
```
...
#### 2. 记忆文件保护 (`memory_protector.py`)
```python
from memory_protector import safe_memory_append, read_memory, auto_maintain

# 安全追加（增量写入，不重写整个文件）
safe_memory_append("2026-03-19", "新的记忆内容...")

# 自动分片（超过500KB自动创建新文件）
# 自动备份（写入前备份，失败时恢复）

# 读取记忆（自动合并分片）
content = read_memory("2026-03-19")
...
**保护策略**:
- 📄 **增量写入** - 只追加新内容，不重写整个文件
- 📦 **自动分片** - 大文件自动拆分（`2026-03-19.md` → `2026-03-19_001.md`）
- 💾 **写入保护** - 写入前备份，失败时自动恢复
- 🗄️ **定期归档** - 7天前的记忆自动归档
...
```python
# 在每条消息处理完成后自动调用
from health_check_v2 import quick_check

if not quick_check():  # 返回False表示有异常
    # 已自动清理，但建议用户注意
    print("⚠️ 检测到资源累积，已自动清理")
```
...
# 记忆维护
python3 memory_protector.py --maintain

# 查看统计
python3 memory_protector.py --stats
```

#### 自动集成
在 `AGENTS.md` 的每次会话启动时:
1. 运行 `health_check_v2.py` 检查状态
2. 运行 `memory_protector.py --maintain` 清理旧文件
3. 重置会话对象基线
...
*维护者: 小爪*
*更新内容: 添加 Chrome MCP 优先使用规则，强化搜索/抓取时的工具选择决策*
